...
export default function UserManagement() {
  ...
  const loadDemoData = async () => {
    if (!confirm('50+ parça ve ilişkili demo veriler Firestore\'a yüklenecek veya güncellenecek. Devam edilsin mi?')) return;
    setSeeding(true);
    try {
      ...
      toast.success(`Demo veri yüklendi: ${summary.parts} parça, ${summary.workOrders} iş emri`);
    } catch (e) {
      console.error(e);
    ...
    <div className="anim-fade" style={{ padding: '24px 28px', maxWidth: 1400, margin: '0 auto' }}>
      <div style={{ marginBottom: 20, padding: 20, background: '#0d1117', border: '1px solid #1e293b', borderRadius: 12 }}>
        <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'flex-start', gap: 20 }}>
          <div>
            <h3 style={{ margin: '0 0 6px', color: '#f1f5f9', fontSize: 18, fontWeight: 800 }}>Demo Veri Merkezi</h3>
            <p style={{ margin: 0, color: '#64748b', fontSize: 13, lineHeight: '1.5' }}>
              Referans projeyi test edebilmeniz için 50+ parça, satınalma, kalite, lot, depo, üretim ve satış verisini tek seferde yükler.
            </p>
          </div>
          <button
          ...
          >
            {seeding ? 'Yükleniyor...' : '50+ Test Verisini Yükle'}
          </button>
        </div>
        ...
        {seedSummary && (
          <div style={{ marginTop: 12, color: '#94a3b8', fontSize: 12, lineHeight: '1.6' }}>
            Örnek sorgular:
            <br />
            Lot: {seedSummary.sampleLotNumbers?.join(', ')}
            <br />
